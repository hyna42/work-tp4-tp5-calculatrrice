...
- `-Wl,-Bstatic -lStaticCal` , intègrer la lib **_libStaticCal.a_** dans l'exécutable
...
- `-Wl,-Bdynamic -lDynamicCal` : permet de charger la lib **_libDynamicCal.so_** au moment de de l'exécution :
  - `-Bdynamic` : repasse en mode lien dynamique
  - `-lDynamicCal` : lie la bibliothèque libDynamicCal.so
- `-Wl,-rpath,'$ORIGIN/../lib/dynamicCalculatrice'` : permet de définir un chemin d’exécution (runtime path) intégré dans l’exécutable afin de pouvoir distribuer l'exécutable avec ses .so sans configurer le système : - `$$ORIGIN` : il s'agit du répertoire où se trouve l’exécutable (../bin/)
  Le programme saura où trouver **_libDynamicCal.so_** au démarrage, **<u>même sans variable LD_LIBRARY_PATH**<u>.
